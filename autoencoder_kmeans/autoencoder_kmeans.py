...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(2006*2005, 1000*1000),
            nn.ReLU(),
            nn.Linear(1000*1000, 500*500),
            nn.ReLU(),
            nn.Linear(500*500,100*100),     # B x 3
        )
        
        self.decoder = nn.Sequential(
            nn.Linear(100*100,500*500),
            nn.ReLU(),
            nn.Linear(500*500,1000*1000),
            nn.ReLU(),
            nn.Linear(1000*1000,2006*2005),     # B x 15
            nn.Sigmoid()
        )
...
encoded_vec = list()
for i in range(encoded_list.shape[1]):
